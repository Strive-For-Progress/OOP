        ...
        if (phase1 > 1) { phase1--; return;}
             ...
             for (unsigned int i=0 ; i<dsts ; i++) {
                    ...
                    while (!Up_order[i].empty() ) {
                        ...
                        ctrl_packet_event(con_id, idx, DstID[i], vertexs[idx].Hash_new[DstID[i]], UpdTime);
                        have_send[i]++;
                        launch = true;
                    }
                    order[i]++;
...
void Dijkstra(unsigned int root,map<pair<unsigned int,unsigned int>,unsigned int>&w ,vector <vertex> &v,short times) {
...
int main() {
    ...
    map<pair<unsigned int,unsigned int>,unsigned int> old_w; // Consider edges to be a value
    map<pair<unsigned int,unsigned int>,unsigned int> new_w; // Consider edges to be a value
    ...
    for (unsigned int id = 0; id < con_id ; id ++){
        for (unsigned int j=0 ; j<dsts ; j++) {
            ...
            ctrl_packet_event(con_id, id, DstID[j], next, InsTime);
            phase1++;
        }
    }

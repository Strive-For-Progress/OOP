...
priority_queue <pair<unsigned int, int>, vector<pair<unsigned int, int> >, greater<pair<unsigned int, int> > > Up_order[100];

map <int,int> convertor;  
unsigned int InsTime,UpdTime,SimDuration;
unsigned int nodes,dsts,links; 
vector <unsigned int> DstID(0);
static int phase1 = 0;
unsigned int con_id ;
class vertex {
    public :
        map<unsigned int,unsigned int> Hash_old; 
        map<unsigned int,unsigned int> Hash_new; 
};
vector <vertex> vertexs(0);
...
class SDN_stat_payload : public payload {
        ...
        vector <pair<unsigned int, unsigned int> > l_info;
    ...
    public:
        ...
        void setInfo(vector<pair<unsigned int, unsigned int> > _info) {
            for (int i=0; i<_info.size() ;i++)    
                 l_info.push_back({_info[i].first,_info[i].second});
        }
        vector<pair<unsigned int, unsigned int> >& getInfo(){return l_info;}
...
class SDN_controller: public node {
    ...
    public:
        ...
        map<pair<unsigned int,unsigned int>,unsigned int> old_w; 
        map<pair<unsigned int,unsigned int>,unsigned int> new_w;
        void Dijkstra(unsigned int ,map<pair<unsigned int,unsigned int>,unsigned int>& ,vector <vertex> &,short);
...
///////Here 5
void stat_packet_event (unsigned int con_id, unsigned int id,vector<pair<unsigned int, unsigned int> >&_info, unsigned int t = event::getCurTime(), string msg = "default"){
// void stat_packet_event (unsigned int dst, unsigned int t = 0, string msg = "default"){
    ...
    e_data.l_info = _info;
    // for (int i=0; i<_info.size() ;i++)     e_data.l_info.push_back({_info[i].first,_info[i].second});
    e_data.msg = msg;
...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_data_packet") { // the switch receives a packet from the other switch
        ...
        if (routing_table.count(p -> getHeader()-> getDstID ()) && getNodeID() !=  routing_table[ p -> getHeader()-> getDstID ()] ) {
            p2->getHeader()->setNexID (routing_table[p->getHeader()->getDstID ()]) ;
        ...
        SDN_switch:: routing_table [l3->getMatID()] =  l3->getActID();
        //reverse src & dst , prev & next
        p3->getHeader()->setPreID (getNodeID());
        // p3->getHeader()->setSrcID (getNodeID()) ;
        p3->getHeader()->setNexID (con_id) ; // ctrl
        p3->getHeader()->setDstID (con_id) ; //ctrl
         send_handler (p3);
    }
...
void SDN_controller::recv_handler (packet *p) {
    static vector<unsigned int> order (dsts,0);
    static vector<unsigned int>  have_send  (dsts,0);
    ...
    if (p == nullptr) return ;
    else if ( p->type() == "SDN_ctrl_packet" && p->getHeader()->getDstID() != con_id) {     
        SDN_ctrl_packet *p3 = nullptr;
        p3 = dynamic_cast<SDN_ctrl_packet*> (p);
        SDN_ctrl_payload *l3 = nullptr;
        l3 = dynamic_cast<SDN_ctrl_payload*> (p3->getPayload());
        p3->getHeader()->setPreID (con_id);
        p3->getHeader()->setNexID (p->getHeader()->getDstID()) ; 
        p3->getHeader()->setDstID (p->getHeader()->getDstID()) ; 
        send_handler (p3);
    }
    else if ( p->type() == "SDN_ctrl_packet" && p->getHeader()->getDstID() == con_id) {   
        if (phase1 >= 1) { phase1--; return;}
        
        SDN_ctrl_packet *p3 = nullptr;
        p3 = dynamic_cast<SDN_ctrl_packet*> (p);
        SDN_ctrl_payload *l3 = nullptr;
        l3 = dynamic_cast<SDN_ctrl_payload*> (p3->getPayload());
            have_send[convertor[l3->getMatID()]]--;

            for (unsigned int i=0 ; i<dsts ; i++) {
                if( have_send[i] == 0) { 
                    bool launch = false ,set = false;
                    while (!Up_order[i].empty()) {
                        unsigned int  _order = Up_order[i].top().first;
                        unsigned int idx = Up_order[i].top().second;
                        if (set == true && launch == false && _order > order[i])  {order[i]++; }//cout<<"Order is increaseing to "<<_order<<"\n";
                        set = true;
                        if (_order == order[i]) Up_order[i].pop();
                        else break;
                        if (idx == DstID[i] ) continue;
                        if (vertexs[idx].Hash_old[DstID[i]] == vertexs[idx].Hash_new[DstID[i]]) continue;
                        ctrl_packet_event(con_id, idx, DstID[i], vertexs[idx].Hash_new[DstID[i]],event::getCurTime());
                        launch = true;
                        have_send[i]++;
                    }
                    order[i]++;  
        ...
        l3 = dynamic_cast<SDN_stat_payload*> (p3->getPayload());
            ...
            if (phase == 0) {
                vector<pair<unsigned int, unsigned int>> read = l3->getInfo();
                 
                for (int i=0; i<read.size() ;i++) {
	                old_w[{p->getHeader()->getSrcID(),read[i].first}]=read[i].second;
                    // cout << "   Node1"      << setw(11) << p->getHeader()->getSrcID()
                    //      << "   Node2"       << setw(11) <<read[i].first
                    //      << "       W"        << setw(11) << read[i].second<<endl;
                }
                rec++;
                if (rec == nodes) is_time = true;   
            }
            else if (phase == 1) {
                vector<pair<unsigned int, unsigned int>> read = l3->getInfo();
                for (int i=0; i<read.size() ;i++) {
	                new_w[{p->getHeader()->getSrcID(),read[i].first}]=read[i].second;
                    // cout << "   Node1"      << setw(11) << p->getHeader()->getSrcID()
                    //      << "   Node2"       << setw(11) <<read[i].first
                    //      << "       W"        << setw(11) << read[i].second<<endl;
                }
                rec--;
                if (rec == 0) is_time = true;
            }
        
        if (is_time){
            if (phase == 0) { 
                for (unsigned int i=0 ; i<dsts ; i++)  Dijkstra(DstID[i],old_w,vertexs,0);
                for (unsigned int id = 0; id < con_id ; id ++) {
                    for (unsigned int j=0 ; j<dsts ; j++) {
                        int next  = vertexs[id].Hash_old[DstID[j]]; 
                        if (id == DstID[j]) continue;
                        ctrl_packet_event(con_id, id, DstID[j], next);
                        phase1++;
                    }
                }
            }
            else if (phase == 1) {
                for (unsigned int i=0 ; i<dsts ; i++)  Dijkstra(DstID[i],new_w,vertexs,1);
                for (unsigned int i=0 ; i<dsts ; i++) {
                    bool launch = false ,set = false;
                    while (!Up_order[i].empty() ) {
                        unsigned int _order = Up_order[i].top().first;
                        unsigned int  idx = Up_order[i].top().second;
                        if (set == true && launch == false && _order > order[i])  {order[i]++; } 
                        set = true;
                        if (_order == order[i]) Up_order[i].pop();
                        else break;
                        if (idx == DstID[i] ) continue;
                        if (vertexs[idx].Hash_old[DstID[i]] == vertexs[idx].Hash_new[DstID[i]]) continue;
                        // cout<<"ctrl ffff"<<"\n";
                        ctrl_packet_event(con_id, idx, DstID[i], vertexs[idx].Hash_new[DstID[i]]);
                        have_send[i]++;
                        launch = true;
                    }
                    order[i]++;
...
void SDN_controller::Dijkstra(unsigned int root,map<pair<unsigned int,unsigned int>,unsigned int>&w ,vector <vertex> &v,short times) {
 
    vector<long long int> dist(node::getNodeNum()-1, 4194967295);  //Ensure not less than any edge
    vector<bool> st(node::getNodeNum()-1, false); //Mean state which is selected or not
    vector<unsigned int> order(node::getNodeNum()-1, 0);

    dist[root]= order[root] = 0; //Start from the destination
    while( times == 1 && !Up_order[convertor[root]].empty())  Up_order[convertor[root]].pop();
    //Select the smallest key with smallest value
    for(unsigned int i=0 ; i<node::getNodeNum()-1 ;i++) {
        unsigned int key = 0;
        bool flag = false;
        for(unsigned int j=0; j<node::getNodeNum()-1 ;j++)
        ...
        if(times == 1) {
            Up_order[convertor[root]].push({order[key],key});
        }

        //Update its neighbors
        const map<unsigned int,bool> &nblist = node::id_to_node(key)-> getPhyNeighbors();
        
        for (map<unsigned int,bool>::const_iterator it = nblist.begin(); it != nblist.end(); it++) {
            if (it->first == con_id) continue;
            unsigned int next = it->first;
            unsigned int cost = w[{key,next}];
            if (dist[next] >  dist[key] + cost ) {
                dist[next] = dist[key] + cost;
                if (times == 0)    { v[next].Hash_old[root] = key;}   
                else if(times == 1) { v[next].Hash_new[root] = key; order[next] = order[key] + 1 ;}   
            } 
            else if (dist[next]  == dist[key] + cost) {
                if (times == 0 &&  v[next].Hash_old[root] > key) { v[next].Hash_old[root] = key;}
                else if(times == 1 && v[next].Hash_new[root] > key)  { v[next].Hash_new[root] = key; order[next] = order[key] + 1 ;}
            }
        }
    }
...
int main() {
    ...
    vertexs.resize(nodes+1);
    
    for (unsigned int i=0,j ; i<dsts ; i++) {
        cin>>j;
        convertor[i] = j; 
        DstID.push_back(j);
    }
...
//
    vector<pair<unsigned int,unsigned int>> fg[nodes];
    vector<pair<unsigned int,unsigned int>> sg[nodes];
    for (unsigned int i=0; i<links; i++) {
        ...
        fg[Node1].push_back({Node2,oldW});
        fg[Node2].push_back({Node1,oldW});
        sg[Node1].push_back({Node2,newW});
        sg[Node2].push_back({Node1,newW});
    }
//
    for (int i=0 ; i<nodes ; i++) stat_packet_event(con_id,i,fg[i],InsTime);
    for (int i=0 ; i<nodes ; i++) stat_packet_event(con_id,i,sg[i],UpdTime);
    ...
    while (cin>>t>>src>>dst)  data_packet_event(src, dst, t);

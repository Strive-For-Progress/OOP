...
void SDN_switch::recv_handler (packet *p){
    ...
    if (routing_table.count(p -> getHeader()-> getDstID ())) {
        ...
        unsigned int dst , next = 0;
        for (int i=0 ; i < msg.length() ; i++) {
            if (msg[i] != ' ') next = next*10 + (msg[i]-'0');
            else {
                dst = next;
                next = 0;
            }
        ...
        //cout<<dst <<" "<<next<<endl;
        SDN_switch:: routing_table [dst] =  next;
    }
...
void Dijkstra(unsigned int root,map<pair<unsigned int,unsigned int>,unsigned int>&w ,vector <vertex> &v,short times) {
    ...
    vector<unsigned int> dist(node::getNodeNum(), 4094967295);  //Ensure not less than any edge
    vector<bool> st(node::getNodeNum(), false); //Mean state which is selected or not
    ...
    for(unsigned int i=0 ; i<node::getNodeNum() ;i++) {
        unsigned int key = 0;
        bool flag = false;
        for(unsigned int j=0; j<node::getNodeNum() ;j++)
        ...
        for (map<unsigned int,bool>::const_iterator it = nblist.begin(); it != nblist.end(); it++) {
            ...
            if (dist[next] >  dist[key] + cost ) {
                ...
                else if(times == 1)  v[next].Hash_new[root] = key;
                ...
                else if(times == 1 && v[next].Hash_new[root] > key)  v[next].Hash_new[root] = key;
            }
        }
    }
...
class buffer{
        public:
         unsigned int t , src , dst ,id;
         int pd;
         string msg;
};

bool cmp (buffer a ,buffer b) {
    if (a.t < b.t)   return true; //time
    if (a.t > b.t)   return false;
    if (a.pd > b.pd) return true;  // crtl : 1 , data : 0
    if (a.pd < b.pd) return false;
    if (a.pd == 1) {                            
    ...
    return true;  //It won't be reaches
}
...
int main()
{
    ...
    for (int i=0 ; i<2 ;i++) {
        ...
        if (i==0) Dijkstra(DstID[j],old_w,vertexs,i);
        else Dijkstra(DstID[j],new_w,vertexs,i);
        }

        for (unsigned int id = 0; id < node::getNodeNum(); id ++){
            for (unsigned int j=0 ; j<dsts ; j++) {
            int next ;
            if(i==0)  next  = vertexs[id].Hash_old[DstID[j]]; 
            else  next  = vertexs[id].Hash_new[DstID[j]];
            if (id == DstID[j] ) continue;
            if (i==1 && vertexs[id].Hash_old[DstID[j]] == vertexs[id].Hash_new[DstID[j]]) continue;
            string msg = to_string(DstID[j]) + " " + to_string(next);
 
            buffer tmp ;
            if (i==0) tmp.t = InsTime ;
            else tmp.t = UpdTime ;
            tmp.msg = msg;
            tmp.id = id;
            tmp.pd = 1;
    ...
    for (int i=0 ; i < store.size() ; i++) {
        if (store[i].pd == 1) {
        ctrl_packet_event(store[i].id, store[i].t, store[i].msg);
        } else {
        data_packet_event(store[i].src, store[i].dst, store[i].t);
        }
      //  if (store[i].pd == 1) {
      //  cout<<"ctrl pACKET Time "<<store[i].t<<" store[i].id"<<" "<<store[i].id<<" "<<" MESG :"<<store[i].msg<<"\n";
      //  } else {
      //  cout<<"data PACKET  TIME: "<<store[i].t<<"src"<<store[i].src<<" "<< store[i].dst<<"\n";
      //  }
    }
